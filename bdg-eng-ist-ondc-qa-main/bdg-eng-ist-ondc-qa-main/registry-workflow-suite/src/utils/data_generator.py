...
class DataGenerator:
    ...
    @staticmethod
    def extract_raw_32byte_key(base64_key: str) -> str:
        """
        Extract raw 32-byte key from base64-encoded Ed25519/X25519 public key.
        
        Ed25519/X25519 public keys in SubjectPublicKeyInfo format have:
        - 12-byte ASN.1 header (0x302a300506032b6570032100 for Ed25519)
        - 32-byte raw public key
        
        This function decodes the base64, skips the ASN.1 header if present, and extracts 
        the raw 32 bytes, then re-encodes to base64.
        
        Args:
            base64_key: Base64-encoded public key (with or without ASN.1 prefix)
        ...
        if not base64_key or not isinstance(base64_key, str):
            raise ValueError(f"Invalid key: {base64_key}")
        
        try:
            # Add padding if missing (common issue with base64)
            padding_needed = (4 - len(base64_key) % 4) % 4
            padded_key = base64_key + ('=' * padding_needed)
            
            decoded = base64.b64decode(padded_key)
            
            # If already 32 bytes, return as-is (already raw)
            if len(decoded) == 32:
                return base64_key
            
            # Standard SubjectPublicKeyInfo is 44 bytes (12-byte header + 32-byte key)
            if len(decoded) == 44:
                raw_key = decoded[12:]  # Skip 12-byte ASN.1 header
                return base64.b64encode(raw_key).decode('utf-8')
            
            # Handle other common formats (extract last 32 bytes)
            if len(decoded) >= 32:
                raw_key = decoded[-32:]
                return base64.b64encode(raw_key).decode('utf-8')
            
            # If less than 32 bytes, key is invalid
            raise ValueError(f"Key too short: {len(decoded)} bytes (need at least 32)")
            
        except binascii.Error as e:
            raise ValueError(f"Invalid base64 encoding: {e}")
        except Exception as e:
            raise ValueError(f"Failed to extract raw key: {e}")
