#!/usr/bin/env python3
"""
ONDC Gateway cancel API - Automated Test Runner with HTML Report Generator

Runs all functional and negative test cases for the ONDC /cancel and /on_cancel
endpoints and generates a comprehensive dark-theme HTML report.

Usage:
    python func_test_scripts/gateway/ondc_gw_cancel_api_tests.py
    python func_test_scripts/gateway/ondc_gw_cancel_api_tests.py --suite functional
    python func_test_scripts/gateway/ondc_gw_cancel_api_tests.py --suite negative
    python func_test_scripts/gateway/ondc_gw_cancel_api_tests.py --timeout 30
    python func_test_scripts/gateway/ondc_gw_cancel_api_tests.py --output reports/my_report.html
    python func_test_scripts/gateway/ondc_gw_cancel_api_tests.py --func-config resources/gateway/ondc_gateway_cancel_functional.yml
    python func_test_scripts/gateway/ondc_gw_cancel_api_tests.py --neg-config  resources/gateway/ondc_gateway_cancel_negative.yml
"""
...
# ---------------------------------------------------------------------------
# Cancel Payload Generator  (/cancel — forward flow)
# ---------------------------------------------------------------------------
class CancelPayloadGenerator:
    def __init__(self, cfg: dict):
        ...
        self.default_order_state = str(cfg.get("default_order_state", "Cancelled"))
    ...
    def items_for_domain(self, domain: str = None) -> List[Dict[str, Any]]:
        ...
        return items or [{"id": "item-001", "name": "Cancelled Item", "price": self.default_order_amount, "category_id": "Generic"}]
    ...
    def _prepare_items(
        ...
        if item_overrides is None:
            source_items = [_json_clone(base_items[0])] if base_items else [{"id": "item-001", "name": "Cancelled Item", "price": self.default_order_amount}]
        ...
        for idx, item in enumerate(source_items, start=1):
            ...
            item_name = str(item.get("name") or item.get("title") or item.get("descriptor", {}).get("name") or f"Cancelled Item {idx}")
            ...
            if not descriptor and item_name:
                descriptor = {"name": item_name, "short_desc": f"{item_name} cancelled"}
...
# ---------------------------------------------------------------------------
# OnCancel Payload Generator  (/on_cancel — backward callback from BPP)
# ---------------------------------------------------------------------------
class OnCancelPayloadGenerator(CancelPayloadGenerator):
    def __init__(self, cfg: dict):
        super().__init__(cfg)
        self.default_on_cancel_state = str(cfg.get("default_on_cancel_state", "Cancelled"))
    ...
    def build(
        ...
        payload = super().build(
            ...
            order_id=order_id,
            order_state=order_state or self.default_on_cancel_state,
        )
        payload["context"]["action"] = "on_cancel"
...
cancelPayloadGenerator = CancelPayloadGenerator
OncancelPayloadGenerator = OnCancelPayloadGenerator
...
def build_test_cases(
    ...
    cpg = CancelPayloadGenerator(func_cfg)
    ocpg = OnCancelPayloadGenerator(func_cfg)
    ncpg = CancelPayloadGenerator(neg_cfg)
    nocpg = OnCancelPayloadGenerator(neg_cfg)

    def _item_overrides(generator: CancelPayloadGenerator, domain: str = None, count: int = 1, quantities: Optional[List[int]] = None, names: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        base_items = generator.items_for_domain(domain)
        if not base_items:
            base_items = [{"id": "item-001", "name": "Cancelled Item", "price": generator.default_order_amount, "category_id": "Generic"}]
        ...
        for idx in range(count):
            ...
            item["id"] = item_id
            item["name"] = names[idx % len(names)] if names else str(item.get("name") or f"Cancelled Item {idx + 1}")
            if quantities:
    ...
    primary_domain = cpg.domains[0] if cpg.domains else ""
    neg_primary_domain = ncpg.domains[0] if ncpg.domains else ""

    # =========================================================================
    # FUNCTIONAL TEST CASES  (/cancel forward + /on_cancel backward)
    # =========================================================================

    # F-TC001 — Valid authenticated /cancel request
    cases.append({
        "id": "F-TC001",
        "name": "cancel Valid Authenticated Request",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends a complete, correctly signed /cancel request with bpp_id and bpp_uri in context (unicast mode). Validates baseline happy-path authentication, payload shape, and Gateway routing for order cancellation messages.",
        "method": "POST",
        "url": f"{func_host}/cancel",
        "payload": cpg.build(),
        "raw_body": None,
    ...
    for domain in cpg.domains:
        cases.append({
            "id": f"F-TC002-{domain}",
            "name": f"cancel Different Domain: {domain}",
            "category": "Functional",
            "description": f"[PASS EXPECTED] Sends /cancel with context.domain='{domain}'. Verifies the Gateway accepts valid cancellation payloads across different ONDC verticals without breaking unicast routing.",
            "method": "POST",
            "url": f"{func_host}/cancel",
            "payload": cpg.build(domain=domain),
            "raw_body": None,
            "raw_content_type": None,
            "expected_status": [200, 202],
            "auth_mode": "valid",
    ...
    for city in cpg.cities:
        cases.append({
            "id": f"F-TC003-{city}",
            "name": f"cancel Different City: {city}",
            "category": "Functional",
            "description": f"[PASS EXPECTED] Sends /cancel with context.city='{city}'. Validates the Gateway accepts cancellation requests for different city codes and preserves location context for fulfillment routing.",
            "method": "POST",
            "url": f"{func_host}/cancel",
            "payload": cpg.build(city=city),
            "raw_body": None,
            "raw_content_type": None,
            "expected_status": [200, 202],
            "auth_mode": "valid",
    ...
    for ft_cfg in cpg.fulfillment_types_for_domain(primary_domain):
        ftype = ft_cfg.get("type", "Delivery") if isinstance(ft_cfg, dict) else str(ft_cfg)
        cases.append({
            "id": f"F-TC004-{ftype}",
            "name": f"cancel Fulfillment Type: {ftype}",
            "category": "Functional",
            "description": f"[PASS EXPECTED] Sends /cancel with order.fulfillments[0].type='{ftype}'. Validates the Gateway accepts cancellation payloads for all configured fulfillment modes.",
            "method": "POST",
            "url": f"{func_host}/cancel",
            "payload": cpg.build(fulfillment_type=ftype),
            "raw_body": None,
            "raw_content_type": None,
            "expected_status": [200, 202],
            "auth_mode": "valid",
    ...
    for pt in cpg.payment_types_for_domain(primary_domain):
        pt_label = pt.get("type", "UNKNOWN") if isinstance(pt, dict) else str(pt)
        cases.append({
            "id": f"F-TC005-{pt_label}",
            "name": f"cancel Payment Type: {pt_label}",
            "category": "Functional",
            "description": f"[PASS EXPECTED] Sends /cancel with order.payment.type='{pt_label}'. Validates the Gateway accepts cancellation payloads covering different payment collection models.",
            "method": "POST",
            "url": f"{func_host}/cancel",
            "payload": cpg.build(payment_type=pt if isinstance(pt, dict) else {"type": str(pt)}),
            "raw_body": None,
            "raw_content_type": None,
            "expected_status": [200, 202],
            "auth_mode": "valid",
    ...
    # F-TC006 — cancel with multiple items
    _multi_item_payload = cpg.build(item_overrides=_item_overrides(cpg, domain=primary_domain, count=3, quantities=[1, 1, 1]))
    cases.append({
        "id": "F-TC006",
        "name": "cancel Multiple Items in Order",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /cancel with multiple order items in a single payload. Validates the Gateway accepts multi-item cancellations and preserves quote breakup alignment with the items list.",
        "method": "POST",
        "url": f"{func_host}/cancel",
    ...
    # F-TC007 — cancel with quantity > 1
    _qty_payload = cpg.build(item_overrides=_item_overrides(cpg, domain=primary_domain, count=1, quantities=[5]))
    cases.append({
        "id": "F-TC007",
        "name": "cancel Quantity Greater Than One",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /cancel where the primary order item has quantity.count > 1. Validates the Gateway preserves item quantity data correctly in the forwarded cancellation payload.",
        "method": "POST",
        "url": f"{func_host}/cancel",
    ...
    for req_num in range(1, 4):
        cases.append({
            ...
            "name": f"cancel Same TxnID / Different MsgID - Req {req_num}",
            "category": "Functional",
            "description": f"[PASS EXPECTED] Sends /cancel request #{req_num} with the same transaction_id but a fresh message_id. Validates the Gateway handles repeated cancellation events within a shared transaction context.",
            "method": "POST",
            "url": f"{func_host}/cancel",
            "payload": cpg.build(transaction_id=_shared_txn),
            "raw_body": None,
            "raw_content_type": None,
            "expected_status": [200, 202],
            "auth_mode": "valid",
    ...
    _unicode_payload = cpg.build(item_overrides=_item_overrides(
        cpg,
    ...
    cases.append({
        "id": "F-TC009",
        "name": "cancel Unicode Item Names",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /cancel with Unicode item descriptors and quote breakup titles in Hindi, Arabic, and Chinese. Validates the Gateway preserves non-ASCII cancellation metadata correctly.",
        "method": "POST",
        "url": f"{func_host}/cancel",
    ...
    _large_cancel_payload = cpg.build(item_overrides=_item_overrides(cpg, domain=primary_domain, count=20))
    cases.append({
        "id": "F-TC010",
        "name": "cancel Large Payload (20 Items)",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /cancel with a large order containing 20 items and a long quote breakup. Validates the Gateway handles large-but-valid cancellation payloads without truncation or timeout.",
        "method": "POST",
        "url": f"{func_host}/cancel",
        "payload": _large_cancel_payload,
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
        "auth_mode": "valid",
        ...
        # F-TC011 — Valid /on_cancel callback with quote
    cases.append({
            "id": "F-TC011",
        "name": "on_cancel Valid Callback With Quote",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends a complete, correctly signed /on_cancel callback with provider, items, billing, fulfillment, payment, and quote breakup. Validates backward-flow callback handling for cancellation messages.",
        "method": "POST",
        "url": f"{func_host}/on_cancel",
        "payload": ocpg.build(),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
    ...
    # F-TC012 — on_cancel empty items
    cases.append({
        "id": "F-TC012",
        "name": "on_cancel Empty Items",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /on_cancel with an order that has an empty items array. Validates the Gateway accepts edge-case callbacks where the cancellation references a temporarily empty order state.",
        "method": "POST",
        "url": f"{func_host}/on_cancel",
        "payload": ocpg.build(empty_items=True),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
        "auth_mode": "valid",
    ...
    # F-TC013 — on_cancel with error block
    cases.append({
        "id": "F-TC013",
        "name": "on_cancel With Error Block",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /on_cancel with a valid order payload plus an error block describing a downstream processing condition. Validates the Gateway forwards error-carrying cancellation callbacks without dropping order context.",
        "method": "POST",
        "url": f"{func_host}/on_cancel",
        "payload": ocpg.build(include_error={"code": "20000", "message": "Cancellation acknowledged with downstream warning"}),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
        "auth_mode": "valid",
    ...
    # F-TC014 — on_cancel multiple providers
    _multi_provider_payload = ocpg.build()
    _providers = ocpg.providers_for_domain(primary_domain)
    _provider_list = []
    ...
    cases.append({
        "id": "F-TC014",
        "name": "on_cancel Multiple Providers Metadata",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /on_cancel with the standard order.provider plus an additional providers array carrying multiple provider references. Validates the Gateway tolerates richer provider metadata in cancellation callbacks.",
        "method": "POST",
        "url": f"{func_host}/on_cancel",
    ...
    # F-TC015 — on_cancel detailed quote breakup
    _detailed_quote_payload = ocpg.build()
    _detailed_quote_payload["message"]["order"]["quote"]["breakup"].extend([
        {
            "@ondc/org/item_id": "tax-001",
            "@ondc/org/title_type": "tax",
            "title": "GST",
            "price": {"currency": ocpg.currency, "value": "18.00"},
        },
        {
            "@ondc/org/item_id": "pack-001",
            "@ondc/org/title_type": "packing",
            "title": "Packing Charges",
            "price": {"currency": ocpg.currency, "value": "12.00"},
        },
        {
            "@ondc/org/item_id": "fee-001",
            "@ondc/org/title_type": "convenience",
            "title": "Convenience Fee",
            "price": {"currency": ocpg.currency, "value": "5.00"},
        },
    ])
    ...
    cases.append({
        "id": "F-TC015",
        "name": "on_cancel Detailed Quote Breakup",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /on_cancel with an expanded quote breakup including tax, packing, and convenience fee lines. Validates the Gateway preserves detailed financial breakup structures in callbacks.",
        "method": "POST",
        "url": f"{func_host}/on_cancel",
    ...
    # F-TC016 — on_cancel different domain (nack_ok if cross-domain)
    _domain2 = ocpg.domains[1] if len(ocpg.domains) > 1 else "ONDC:RET16"
    _domain2_is_cross = _domain2 not in set(ocpg.domains)
    _domain2_desc = (
        f"[PASS EXPECTED] Sends /on_cancel with context.domain='{_domain2}'. "
        + (
            ...
            "Validates the Gateway accepts cancellation callbacks for non-primary configured domains."
        )
    )
    cases.append({
        "id": "F-TC016",
        "name": f"on_cancel Different Domain: {_domain2}",
        "category": "Functional",
        "description": _domain2_desc,
        "method": "POST",
        "url": f"{func_host}/on_cancel",
        "payload": ocpg.build(domain=_domain2),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
        "auth_mode": "valid",
    ...
    # F-TC017 — on_cancel large order (25 items)
    _large_on_cancel_payload = ocpg.build(item_overrides=_item_overrides(ocpg, domain=primary_domain, count=25))
    cases.append({
        "id": "F-TC017",
        "name": "on_cancel Large Order (25 Items)",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /on_cancel with a large order containing 25 items. Validates the Gateway accepts and forwards large callback payloads without truncating item or quote breakup data.",
        "method": "POST",
        "url": f"{func_host}/on_cancel",
        "payload": _large_on_cancel_payload,
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
        "auth_mode": "valid",
    ...
    cases.append({
        "id": "N-TC001",
        "name": "cancel Missing Authorization Header",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Sends /cancel with Content-Type but no Authorization header. Gateway must reject unauthenticated ONDC cancellation requests.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": ncpg.build(),
        "raw_body": None,
        "raw_content_type": None,
    ...
    cases.append({
        "id": "N-TC002",
        "name": "cancel Invalid / Tampered Signature",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Sends /cancel with a valid-looking Authorization header whose signature is intentionally tampered. Gateway must reject cryptographically invalid signatures.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": ncpg.build(),
        "raw_body": None,
    ...
    _no_domain = ncpg.build()
    _no_domain["context"].pop("domain", None)
    cases.append({
        "id": "N-TC003",
        "name": "cancel Missing context.domain",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /cancel with context.domain removed. Domain is mandatory for Gateway validation and routing. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": _no_domain,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC004",
        "name": "cancel Missing context.bap_id",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /cancel with bap_id removed from context. Without bap_id, Gateway cannot identify the requesting buyer application. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": _no_bap,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC005",
        "name": "cancel Missing context.bpp_id",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /cancel with bpp_id and bpp_uri removed from context. cancel is always unicast, so the Gateway must reject a payload that does not identify the target BPP. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": _no_bpp,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC006",
        "name": "cancel Expired Signature (TTL = 10s)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Generates a /cancel signature with TTL=10s then waits 12 seconds before sending. Gateway must reject requests whose authentication signature is already expired.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": ncpg.build(),
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC007",
        "name": "cancel Invalid core_version (99.abcd)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends /cancel with an invalid core_version value. Gateway must reject requests referencing unsupported protocol versions.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": _bad_ver,
        "raw_body": None,
    ...
    _bad_domain = ncpg.build()
    _bad_domain["context"]["domain"] = "INVALID:DOMAIN999"
    cases.append({
        "id": "N-TC008",
        "name": "cancel Invalid Domain (INVALID:DOMAIN999)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends /cancel with an unrecognised domain code. Gateway must reject requests for unsupported ONDC domains.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": _bad_domain,
        "raw_body": None,
    ...
    _bad_city = ncpg.build()
    _bad_city["context"]["city"] = "invalid:city:format"
    cases.append({
        "id": "N-TC009",
        "name": "cancel Invalid City Code (invalid:city:format)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /cancel with a malformed city code. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": _bad_city,
        "raw_body": None,
    ...
    _bad_country = ncpg.build()
    _bad_country["context"]["country"] = "XYZZY"
    cases.append({
        "id": "N-TC010",
        "name": "cancel Invalid Country Code (XYZZY)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /cancel with an invalid country code. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
    ...
    _bad_shape = ncpg.build()
    _bad_shape["message"] = {"unexpected": "missing order block"}
    cases.append({
        "id": "N-TC011",
        "name": "cancel Malformed JSON Structure (missing message.order)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /cancel with valid JSON but the wrong ONDC shape (message.order missing). Gateway must reject structurally invalid cancellation payloads. May return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": _bad_shape,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC012",
        "name": "cancel Invalid JSON Syntax (raw string)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends POST /cancel with Content-Type application/json but body is '{not valid json'. Gateway must reject unparseable JSON.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": None,
    ...
    cases.append({
        "id": "N-TC013",
        "name": "cancel Empty JSON Payload ({})",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends POST /cancel with an empty JSON body {}. Gateway must reject payloads with no context or message data.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": {},
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC014",
        "name": "cancel Wrong HTTP Method (GET)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 404/405] Sends the /cancel request as GET instead of POST. ONDC Gateway cancel endpoints only accept POST.",
        "method": "GET",
        "url": f"{neg_host}/cancel",
        "payload": None,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC015",
        "name": "cancel Wrong Content-Type (text/plain)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends /cancel with Content-Type: text/plain instead of application/json. Gateway should reject invalid content types for ONDC payloads.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": None,
    ...
    cases.append({
        "id": "N-TC016",
        "name": "cancel Invalid Authorization Header Format",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Sends /cancel with Authorization: Bearer token_xyz instead of the ONDC Signature format. Gateway must reject non-ONDC auth schemes.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": ncpg.build(),
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC017",
        "name": "cancel Missing context.transaction_id",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /cancel with transaction_id removed from context. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": _no_txn,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC018",
        "name": "cancel Missing context.timestamp",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /cancel with context.timestamp removed. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
        "payload": _no_ts,
        "raw_body": None,
    ...
    _dos_items = _item_overrides(ncpg, domain=neg_primary_domain, count=100)
    for idx, item in enumerate(_dos_items, start=1):
        huge = "x" * 500
        item["name"] = f"DoS Item {idx} {huge}"
        item["descriptor"] = {"name": item["name"], "short_desc": huge}
    _dos_payload = ncpg.build(item_overrides=_dos_items)
    cases.append({
        "id": "N-TC019",
        "name": "cancel Extremely Large Payload (DoS / Size Limit)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /cancel with 100 items whose descriptors are intentionally oversized. Tests request size limits and DoS protection. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
    ...
    _wrong_action = ncpg.build()
    _wrong_action["context"]["action"] = "confirm"
    cases.append({
        "id": "N-TC020",
        "name": "cancel Wrong action Value (confirm on /cancel endpoint)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends POST /cancel with context.action='confirm' instead of 'cancel'. Gateway must validate the action field. May return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/cancel",
    ...
    # N-TC021 — on_cancel missing Authorization header
    cases.append({
        "id": "N-TC021",
        "name": "on_cancel Missing Authorization Header",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Sends /on_cancel with no Authorization header. Backward callbacks must also be authenticated.",
        "method": "POST",
        "url": f"{neg_host}/on_cancel",
        "payload": nocpg.build(),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [401, 400],
    ...
    # N-TC022 — on_cancel tampered signature
    cases.append({
        "id": "N-TC022",
        "name": "on_cancel Tampered Signature",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Sends /on_cancel with a cryptographically invalid signature. Gateway must verify the BPP's signature before accepting the callback.",
        "method": "POST",
        "url": f"{neg_host}/on_cancel",
        "payload": nocpg.build(),
        "raw_body": None,
    ...
    # N-TC023 — on_cancel missing context.bpp_id
    cases.append({
        "id": "N-TC023",
        "name": "on_cancel Missing context.bpp_id",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends /on_cancel without bpp_id and bpp_uri in context. Gateway cannot identify the sender BPP without these fields.",
        "method": "POST",
        "url": f"{neg_host}/on_cancel",
        "payload": nocpg.build(omit_bpp_id=True),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [400, 401],
        "auth_mode": "valid",
    ...
    # N-TC024 — on_cancel missing context.bap_id
    cases.append({
        "id": "N-TC024",
        "name": "on_cancel Missing context.bap_id",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /on_cancel without bap_id and bap_uri in context. Gateway may return 4xx or HTTP 200 NACK because it cannot route the callback to the target BAP.",
        "method": "POST",
        "url": f"{neg_host}/on_cancel",
    ...
    # N-TC025 — on_cancel missing context.domain
    cases.append({
        "id": "N-TC025",
        "name": "on_cancel Missing context.domain",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /on_cancel with domain removed from context. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/on_cancel",
        "payload": nocpg.build(omit_domain=True),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [400, 401],
        "auth_mode": "valid",
    ...
    # N-TC026 — on_cancel wrong HTTP method (GET)
    cases.append({
        "id": "N-TC026",
        "name": "on_cancel Wrong HTTP Method (GET)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 404/405] Sends the /on_cancel request as GET instead of POST. ONDC Gateway callback endpoints only accept POST.",
        "method": "GET",
        "url": f"{neg_host}/on_cancel",
        "payload": None,
        "raw_body": None,
...
<title>ONDC Gateway cancel API Test Report</title>
<style>
...
<div class="page">
  ...
  <div class="hero">
    <h1>ONDC <span>Gateway cancel API</span> Test Report</h1>
    <p>
      <strong>Source:</strong> Gateway /cancel &amp; /on_cancel — Functional &amp; Negative
      &nbsp;|&nbsp;
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ONDC Gateway cancel API Test Runner",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Output HTML path. If omitted, auto-generated as reports/Gateway-cancel-<suite>-<timestamp>.html",
    )
    parser.add_argument(
        "--suite",
    ...
    parser.add_argument(
        "--func-config",
        default="resources/gateway/ondc_gateway_cancel_functional.yml",
        help="Path to functional YAML config (relative to project root or absolute)",
    )
    parser.add_argument(
        "--neg-config",
        default="resources/gateway/ondc_gateway_cancel_negative.yml",
    ...
    if args.output:
        ...
        output_path = os.path.join(
            ...
            f"Gateway-cancel-{args.suite}-{ts_file}.html",
        )

    logger.info("=" * 60)
    logger.info("ONDC Gateway cancel API Test Runner")
