#!/usr/bin/env python3
"""
ONDC Gateway track API - Automated Test Runner with HTML Report Generator

Runs all functional and negative test cases for the ONDC /track and /on_track
endpoints and generates a comprehensive dark-theme HTML report.

Usage:
    python func_test_scripts/gateway/ondc_gw_track_api_tests.py
    python func_test_scripts/gateway/ondc_gw_track_api_tests.py --suite functional
    python func_test_scripts/gateway/ondc_gw_track_api_tests.py --suite negative
    python func_test_scripts/gateway/ondc_gw_track_api_tests.py --timeout 30
    python func_test_scripts/gateway/ondc_gw_track_api_tests.py --output reports/my_report.html
    python func_test_scripts/gateway/ondc_gw_track_api_tests.py --func-config resources/gateway/ondc_gateway_track_functional.yml
    python func_test_scripts/gateway/ondc_gw_track_api_tests.py --neg-config  resources/gateway/ondc_gateway_track_negative.yml
"""
...
# ---------------------------------------------------------------------------
# track Payload Generator  (/track — forward flow)
# ---------------------------------------------------------------------------
class TrackPayloadGenerator:
    def __init__(self, cfg: dict):
        ...
        self.default_order_state = str(cfg.get("default_order_state", "Tracking Requested"))
    ...
    def items_for_domain(self, domain: str = None) -> List[Dict[str, Any]]:
        ...
        return items or [{"id": "item-001", "name": "Tracked Item", "price": self.default_order_amount, "category_id": "Generic"}]
    ...
    def _prepare_items(
        ...
        if item_overrides is None:
            source_items = [_json_clone(base_items[0])] if base_items else [{"id": "item-001", "name": "Tracked Item", "price": self.default_order_amount}]
        ...
        for idx, item in enumerate(source_items, start=1):
            ...
            item_name = str(item.get("name") or item.get("title") or item.get("descriptor", {}).get("name") or f"Tracked Item {idx}")
            ...
            if not descriptor and item_name:
                descriptor = {"name": item_name, "short_desc": f"{item_name} tracking request"}
...
# ---------------------------------------------------------------------------
# on_track Payload Generator  (/on_track — backward callback from BPP)
# ---------------------------------------------------------------------------
class OnTrackPayloadGenerator(TrackPayloadGenerator):
    def __init__(self, cfg: dict):
        super().__init__(cfg)
        self.default_on_track_state = str(cfg.get("default_on_track_state", "Tracking Active"))
    ...
    def build(
        ...
        payload = super().build(
            ...
            payment_type=payment_type,
            fulfillment_type=fulfillment_type,
            order_state=order_state or self.default_on_track_state,
            order_id=order_id,
        )
        payload["context"]["action"] = "on_track"
...
trackPayloadGenerator = TrackPayloadGenerator
OntrackPayloadGenerator = OnTrackPayloadGenerator
...
def build_test_cases(
    ...
    tpg = TrackPayloadGenerator(func_cfg)
    otpg = OnTrackPayloadGenerator(func_cfg)
    ntpg = TrackPayloadGenerator(neg_cfg)
    notpg = OnTrackPayloadGenerator(neg_cfg)

    def _item_overrides(generator: TrackPayloadGenerator, domain: str = None, count: int = 1, quantities: Optional[List[int]] = None, names: Optional[List[str]] = None) -> List[Dict[str, Any]]:
        base_items = generator.items_for_domain(domain)
        if not base_items:
            base_items = [{"id": "item-001", "name": "Tracked Item", "price": generator.default_order_amount, "category_id": "Generic"}]
        ...
        for idx in range(count):
            ...
            item["id"] = item_id
            item["name"] = names[idx % len(names)] if names else str(item.get("name") or f"Tracked Item {idx + 1}")
            if quantities:
    ...
    primary_domain = tpg.domains[0] if tpg.domains else ""
    neg_primary_domain = ntpg.domains[0] if ntpg.domains else ""

    # =========================================================================
    # FUNCTIONAL TEST CASES  (/track forward + /on_track backward)
    # =========================================================================

    # F-TC001 — Valid authenticated /track request
    cases.append({
        "id": "F-TC001",
        "name": "track Valid Authenticated Request",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends a complete, correctly signed /track request with bpp_id and bpp_uri in context (unicast mode). Validates baseline happy-path authentication, payload shape, and Gateway routing for order tracking messages.",
        "method": "POST",
        "url": f"{func_host}/track",
        "payload": tpg.build(),
        "raw_body": None,
    ...
    for domain in tpg.domains:
        cases.append({
            "id": f"F-TC002-{domain}",
            "name": f"track Different Domain: {domain}",
            "category": "Functional",
            "description": f"[PASS EXPECTED] Sends /track with context.domain='{domain}'. Verifies the Gateway accepts valid tracking payloads across different ONDC verticals without breaking unicast routing.",
            "method": "POST",
            "url": f"{func_host}/track",
            "payload": tpg.build(domain=domain),
            "raw_body": None,
            "raw_content_type": None,
            "expected_status": [200, 202],
            "auth_mode": "valid",
    ...
    for city in tpg.cities:
        cases.append({
            "id": f"F-TC003-{city}",
            "name": f"track Different City: {city}",
            "category": "Functional",
            "description": f"[PASS EXPECTED] Sends /track with context.city='{city}'. Validates the Gateway accepts tracking requests for different city codes and preserves location context for fulfillment routing.",
            "method": "POST",
            "url": f"{func_host}/track",
            "payload": tpg.build(city=city),
            "raw_body": None,
            "raw_content_type": None,
            "expected_status": [200, 202],
            "auth_mode": "valid",
    ...
    for ft_cfg in tpg.fulfillment_types_for_domain(primary_domain):
        ftype = ft_cfg.get("type", "Delivery") if isinstance(ft_cfg, dict) else str(ft_cfg)
        cases.append({
            "id": f"F-TC004-{ftype}",
            "name": f"track Fulfillment Type: {ftype}",
            "category": "Functional",
            "description": f"[PASS EXPECTED] Sends /track with order.fulfillment.type='{ftype}'. Validates the Gateway accepts tracking payloads for all configured fulfillment modes.",
            "method": "POST",
            "url": f"{func_host}/track",
            "payload": tpg.build(fulfillment_type=ftype),
            "raw_body": None,
            "raw_content_type": None,
            "expected_status": [200, 202],
            "auth_mode": "valid",
    ...
    for pt in tpg.payment_types_for_domain(primary_domain):
        pt_label = pt.get("type", "UNKNOWN") if isinstance(pt, dict) else str(pt)
        cases.append({
            "id": f"F-TC005-{pt_label}",
            "name": f"track Payment Type: {pt_label}",
            "category": "Functional",
            "description": f"[PASS EXPECTED] Sends /track with order.payment.type='{pt_label}'. Validates the Gateway accepts tracking payloads covering different payment collection models.",
            "method": "POST",
            "url": f"{func_host}/track",
            "payload": tpg.build(payment_type=pt if isinstance(pt, dict) else {"type": str(pt)}),
            "raw_body": None,
            "raw_content_type": None,
            "expected_status": [200, 202],
            "auth_mode": "valid",
    ...
    # F-TC006 — track with multiple items
    _multi_item_payload = tpg.build(item_overrides=_item_overrides(tpg, domain=primary_domain, count=3, quantities=[1, 1, 1]))
    cases.append({
        "id": "F-TC006",
        "name": "track Multiple Items in Order",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /track with multiple order items in a single payload. Validates the Gateway accepts multi-item tracking requests and preserves quote breakup alignment with the items list.",
        "method": "POST",
        "url": f"{func_host}/track",
    ...
    # F-TC007 — track with quantity > 1
    _qty_payload = tpg.build(item_overrides=_item_overrides(tpg, domain=primary_domain, count=1, quantities=[5]))
    cases.append({
        "id": "F-TC007",
        "name": "track Quantity Greater Than One",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /track where the primary order item has quantity.count > 1. Validates the Gateway preserves item quantity data correctly in the forwarded tracking payload.",
        "method": "POST",
        "url": f"{func_host}/track",
    ...
    for req_num in range(1, 4):
        cases.append({
            ...
            "name": f"track Same TxnID / Different MsgID - Req {req_num}",
            "category": "Functional",
            "description": f"[PASS EXPECTED] Sends /track request #{req_num} with the same transaction_id but a fresh message_id. Validates the Gateway handles repeated tracking events within a shared transaction context.",
            "method": "POST",
            "url": f"{func_host}/track",
            "payload": tpg.build(transaction_id=_shared_txn),
            "raw_body": None,
            "raw_content_type": None,
            "expected_status": [200, 202],
            "auth_mode": "valid",
    ...
    _unicode_payload = tpg.build(item_overrides=_item_overrides(
        tpg,
    ...
    cases.append({
        "id": "F-TC009",
        "name": "track Unicode Item Names",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /track with Unicode item descriptors and quote breakup titles in Hindi, Arabic, and Chinese. Validates the Gateway preserves non-ASCII tracking metadata correctly.",
        "method": "POST",
        "url": f"{func_host}/track",
    ...
    _large_track_payload = tpg.build(item_overrides=_item_overrides(tpg, domain=primary_domain, count=20))
    cases.append({
        "id": "F-TC010",
        "name": "track Large Payload (20 Items)",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /track with a large order containing 20 items and a long quote breakup. Validates the Gateway handles large-but-valid tracking payloads without truncation or timeout.",
        "method": "POST",
        "url": f"{func_host}/track",
        "payload": _large_track_payload,
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
        "auth_mode": "valid",
    ...
    cases.append({
        ...
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends GET / to verify the Gateway service is reachable before the track suite runs. Some Gateway deployments return 404 at the root path while still proving network reachability, so 404 is treated as an acceptable smoke-check response.",
        "method": "GET",
        "url": f"{func_host}/",
        "payload": None,
        "raw_body": None,
    ...
    # F-TC012 — Valid /on_track callback with quote
    cases.append({
        "id": "F-TC012",
        "name": "on_track Valid Callback With Quote",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends a complete, correctly signed /on_track callback with provider, items, billing, fulfillment, payment, and quote breakup. Validates backward-flow callback handling for order tracking messages.",
        "method": "POST",
        "url": f"{func_host}/on_track",
        "payload": otpg.build(),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
    ...
    # F-TC013 — on_track empty items
    cases.append({
        "id": "F-TC013",
        "name": "on_track Empty Items",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /on_track with an order that has an empty items array. Validates the Gateway accepts edge-case callbacks where the tracking response references a temporarily empty order state.",
        "method": "POST",
        "url": f"{func_host}/on_track",
        "payload": otpg.build(empty_items=True),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
        "auth_mode": "valid",
    ...
    # F-TC014 — on_track with error block
    cases.append({
        "id": "F-TC014",
        "name": "on_track With Error Block",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /on_track with a valid order payload plus an error block describing a downstream tracking condition. Validates the Gateway forwards error-carrying tracking callbacks without dropping order context.",
        "method": "POST",
        "url": f"{func_host}/on_track",
        "payload": otpg.build(include_error={"code": "20000", "message": "Tracking acknowledged with downstream warning"}),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
        "auth_mode": "valid",
    ...
    # F-TC015 — on_track multiple providers
    _multi_provider_payload = otpg.build()
    _providers = otpg.providers_for_domain(primary_domain)
    _provider_list = []
    ...
    cases.append({
        "id": "F-TC015",
        "name": "on_track Multiple Providers Metadata",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /on_track with the standard order.provider plus an additional providers array carrying multiple provider references. Validates the Gateway tolerates richer provider metadata in tracking callback payloads.",
        "method": "POST",
        "url": f"{func_host}/on_track",
    ...
    # F-TC016 — on_track detailed quote breakup
    _detailed_quote_payload = otpg.build()
    _detailed_quote_payload["message"]["order"]["quote"]["breakup"].extend([
        {
            "@ondc/org/item_id": "tax-001",
            "@ondc/org/title_type": "tax",
            "title": "GST",
            "price": {"currency": otpg.currency, "value": "18.00"},
        },
        {
            "@ondc/org/item_id": "track-001",
            "@ondc/org/title_type": "tracking",
            "title": "Tracking Fee",
            "price": {"currency": otpg.currency, "value": "12.00"},
        },
        {
            "@ondc/org/item_id": "fee-001",
            "@ondc/org/title_type": "convenience",
            "title": "Convenience Fee",
            "price": {"currency": otpg.currency, "value": "5.00"},
        },
    ])
    ...
    cases.append({
        "id": "F-TC016",
        "name": "on_track Detailed Quote Breakup",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /on_track with an expanded quote breakup including tax, tracking fee, and convenience fee lines. Validates the Gateway preserves detailed financial breakup structures in callbacks.",
        "method": "POST",
        "url": f"{func_host}/on_track",
    ...
    # F-TC017 — on_track different domain (nack_ok if cross-domain)
    _domain2 = otpg.domains[1] if len(otpg.domains) > 1 else "ONDC:RET16"
    _domain2_is_cross = _domain2 not in set(otpg.domains)
    _domain2_desc = (
        f"[PASS EXPECTED] Sends /on_track with context.domain='{_domain2}'. "
        + (
            ...
            "Validates the Gateway accepts tracking callbacks for non-primary configured domains."
        )
    )
    cases.append({
        "id": "F-TC017",
        "name": f"on_track Different Domain: {_domain2}",
        "category": "Functional",
        "description": _domain2_desc,
        "method": "POST",
        "url": f"{func_host}/on_track",
        "payload": otpg.build(domain=_domain2),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
        "auth_mode": "valid",
    ...
    # F-TC018 — on_track large order (25 items)
    _large_on_track_payload = otpg.build(item_overrides=_item_overrides(otpg, domain=primary_domain, count=25))
    cases.append({
        "id": "F-TC018",
        "name": "on_track Large Order (25 Items)",
        "category": "Functional",
        "description": "[PASS EXPECTED] Sends /on_track with a large order containing 25 items. Validates the Gateway accepts and forwards large callback payloads without truncating item or quote breakup data.",
        "method": "POST",
        "url": f"{func_host}/on_track",
        "payload": _large_on_track_payload,
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [200, 202],
        "auth_mode": "valid",
    ...
    cases.append({
        "id": "N-TC001",
        "name": "track Missing Authorization Header",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Sends /track with Content-Type but no Authorization header. Gateway must reject unauthenticated ONDC tracking requests.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": ntpg.build(),
        "raw_body": None,
        "raw_content_type": None,
    ...
    cases.append({
        "id": "N-TC002",
        "name": "track Invalid / Tampered Signature",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Sends /track with a valid-looking Authorization header whose signature is intentionally tampered. Gateway must reject cryptographically invalid signatures.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": ntpg.build(),
        "raw_body": None,
    ...
    _no_domain = ntpg.build()
    _no_domain["context"].pop("domain", None)
    cases.append({
        "id": "N-TC003",
        "name": "track Missing context.domain",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /track with context.domain removed. Domain is mandatory for Gateway validation and routing. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": _no_domain,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC004",
        "name": "track Missing context.bap_id",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /track with bap_id removed from context. Without bap_id, Gateway cannot identify the requesting buyer application. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": _no_bap,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC005",
        "name": "track Missing context.bpp_id",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /track with bpp_id and bpp_uri removed from context. track is always unicast, so the Gateway must reject a payload that does not identify the target BPP. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": _no_bpp,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC006",
        "name": "track Expired Signature (TTL = 10s)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Generates a /track signature with TTL=10s then waits 12 seconds before sending. Gateway must reject requests whose authentication signature is already expired.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": ntpg.build(),
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC007",
        "name": "track Invalid core_version (99.abcd)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends /track with an invalid core_version value. Gateway must reject requests referencing unsupported protocol versions.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": _bad_ver,
        "raw_body": None,
    ...
    _bad_domain = ntpg.build()
    _bad_domain["context"]["domain"] = "INVALID:DOMAIN999"
    cases.append({
        "id": "N-TC008",
        "name": "track Invalid Domain (INVALID:DOMAIN999)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends /track with an unrecognised domain code. Gateway must reject requests for unsupported ONDC domains.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": _bad_domain,
        "raw_body": None,
    ...
    _bad_city = ntpg.build()
    _bad_city["context"]["city"] = "invalid:city:format"
    cases.append({
        "id": "N-TC009",
        "name": "track Invalid City Code (invalid:city:format)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /track with a malformed city code. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": _bad_city,
        "raw_body": None,
    ...
    _bad_country = ntpg.build()
    _bad_country["context"]["country"] = "XYZZY"
    cases.append({
        "id": "N-TC010",
        "name": "track Invalid Country Code (XYZZY)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /track with an invalid country code. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/track",
    ...
    _bad_shape = ntpg.build()
    _bad_shape["message"] = {"unexpected": "missing order block"}
    cases.append({
        "id": "N-TC011",
        "name": "track Malformed JSON Structure (missing message.order)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /track with valid JSON but the wrong ONDC shape (message.order missing). Gateway must reject structurally invalid tracking payloads. May return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": _bad_shape,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC012",
        "name": "track Invalid JSON Syntax (raw string)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends POST /track with Content-Type application/json but body is '{not valid json'. Gateway must reject unparseable JSON.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": None,
    ...
    cases.append({
        "id": "N-TC013",
        "name": "track Empty JSON Payload ({})",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends POST /track with an empty JSON body {}. Gateway must reject payloads with no context or message data.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": {},
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC014",
        "name": "track Wrong HTTP Method (GET)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 404/405] Sends the /track request as GET instead of POST. ONDC Gateway track endpoints only accept POST.",
        "method": "GET",
        "url": f"{neg_host}/track",
        "payload": None,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC015",
        "name": "track Wrong Content-Type (text/plain)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends /track with Content-Type: text/plain instead of application/json. Gateway should reject invalid content types for ONDC payloads.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": None,
    ...
    cases.append({
        "id": "N-TC016",
        "name": "track Invalid Authorization Header Format",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Sends /track with Authorization: Bearer token_xyz instead of the ONDC Signature format. Gateway must reject non-ONDC auth schemes.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": ntpg.build(),
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC017",
        "name": "track Missing context.transaction_id",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /track with transaction_id removed from context. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": _no_txn,
        "raw_body": None,
    ...
    cases.append({
        "id": "N-TC018",
        "name": "track Missing context.timestamp",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /track with context.timestamp removed. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/track",
        "payload": _no_ts,
        "raw_body": None,
    ...
    _dos_items = _item_overrides(ntpg, domain=neg_primary_domain, count=100)
    for idx, item in enumerate(_dos_items, start=1):
        huge = "x" * 500
        item["name"] = f"DoS Item {idx} {huge}"
        item["descriptor"] = {"name": item["name"], "short_desc": huge}
    _dos_payload = ntpg.build(item_overrides=_dos_items)
    cases.append({
        "id": "N-TC019",
        "name": "track Extremely Large Payload (DoS / Size Limit)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /track with 100 items whose descriptors are intentionally oversized. Tests request size limits and DoS protection. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/track",
    ...
    _wrong_action = ntpg.build()
    _wrong_action["context"]["action"] = "confirm"
    cases.append({
        "id": "N-TC020",
        "name": "track Wrong action Value (confirm on /track endpoint)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends POST /track with context.action='confirm' instead of 'track'. Gateway must validate the action field. May return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/track",
    ...
    # N-TC021 — on_track missing Authorization header
    cases.append({
        "id": "N-TC021",
        "name": "on_track Missing Authorization Header",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Sends /on_track with no Authorization header. Backward callbacks must also be authenticated.",
        "method": "POST",
        "url": f"{neg_host}/on_track",
        "payload": notpg.build(),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [401, 400],
    ...
    # N-TC022 — on_track tampered signature
    cases.append({
        "id": "N-TC022",
        "name": "on_track Tampered Signature",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401] Sends /on_track with a cryptographically invalid signature. Gateway must verify the BPP's signature before accepting the callback.",
        "method": "POST",
        "url": f"{neg_host}/on_track",
        "payload": notpg.build(),
        "raw_body": None,
    ...
    # N-TC023 — on_track missing context.bpp_id
    cases.append({
        "id": "N-TC023",
        "name": "on_track Missing context.bpp_id",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400] Sends /on_track without bpp_id and bpp_uri in context. Gateway cannot identify the sender BPP without these fields.",
        "method": "POST",
        "url": f"{neg_host}/on_track",
        "payload": notpg.build(omit_bpp_id=True),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [400, 401],
        "auth_mode": "valid",
    ...
    # N-TC024 — on_track missing context.bap_id
    cases.append({
        "id": "N-TC024",
        "name": "on_track Missing context.bap_id",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /on_track without bap_id and bap_uri in context. Gateway may return 4xx or HTTP 200 NACK because it cannot route the callback to the target BAP.",
        "method": "POST",
        "url": f"{neg_host}/on_track",
        "payload": notpg.build(omit_bap_id=True),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [400, 401],
        "auth_mode": "valid",
    ...
    # N-TC025 — on_track missing context.domain
    cases.append({
        "id": "N-TC025",
        "name": "on_track Missing context.domain",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 401/400/NACK] Sends /on_track with domain removed from context. Gateway may return 4xx or HTTP 200 NACK.",
        "method": "POST",
        "url": f"{neg_host}/on_track",
        "payload": notpg.build(omit_domain=True),
        "raw_body": None,
        "raw_content_type": None,
        "expected_status": [400, 401],
        "auth_mode": "valid",
    ...
    # N-TC026 — on_track wrong HTTP method (GET)
    cases.append({
        "id": "N-TC026",
        "name": "on_track Wrong HTTP Method (GET)",
        "category": "Negative",
        "description": "[FAIL EXPECTED — 404/405] Sends the /on_track request as GET instead of POST. ONDC Gateway callback endpoints only accept POST.",
        "method": "GET",
        "url": f"{neg_host}/on_track",
        "payload": None,
        "raw_body": None,
...
<title>ONDC Gateway track API Test Report</title>
<style>
...
<div class="page">
  ...
  <div class="hero">
    <h1>ONDC <span>Gateway track API</span> Test Report</h1>
    <p>
      <strong>Source:</strong> Gateway /track &amp; /on_track — Functional &amp; Negative
      &nbsp;|&nbsp;
...
def _reg_build_payload(cfg: dict, signing_pub: str, enc_pub: str) -> dict:
    ...
    reg_slug = _slugify(participant_id)
    location_id = str(cfg.get("registration_location_id", f"loc-{reg_slug}-001"))
    uri_id = str(cfg.get("registration_uri_id", f"uri-{reg_slug}-001"))
    cred_id = str(cfg.get("registration_cred_id", f"cred-gst-{reg_slug}-001"))
    contact_id = str(cfg.get("registration_contact_id", f"contact-tech-{reg_slug}-001"))
    configs = [
    ...
    return {
        ...
        "action": "SUBSCRIBED",
        "credentials": [{"cred_id": cred_id, "type": "GST",
                         "cred_data": {"gstin": "29ABCDE1234F1Z5",
                                       "legal_name": "Gateway Test BAP Pvt Ltd"}}],
        "contacts": [{"contact_id": contact_id, "type": "TECHNICAL",
...
def register_bap_participant(cfg: dict) -> None:
    ...
    timeout_seconds = _safe_int(cfg.get("registration_timeout_seconds", 15), 15)
    ...
    try:
        resp = requests.post(url, data=wire.encode(), headers=headers,
                             timeout=timeout_seconds, verify=_SSL_VERIFY)
        body = resp.text
    except Exception as exc:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ONDC Gateway track API Test Runner",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Output HTML path. If omitted, auto-generated as reports/Gateway-track-<suite>-<timestamp>.html",
    )
    parser.add_argument(
        "--suite",
    ...
    parser.add_argument(
        "--func-config",
        default="resources/gateway/ondc_gateway_track_functional.yml",
        help="Path to functional YAML config (relative to project root or absolute)",
    )
    parser.add_argument(
        "--neg-config",
        default="resources/gateway/ondc_gateway_track_negative.yml",
    ...
    if args.output:
        ...
        output_path = os.path.join(
            ...
            f"Gateway-track-{args.suite}-{ts_file}.html",
        )

    logger.info("=" * 60)
    logger.info("ONDC Gateway track API Test Runner")
