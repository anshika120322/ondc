"""
ONDC Registry – Endpoint Accessibility Tests (Multi-Environment Comparison)
===========================================================================
...
CONFIG_PATH = WORKSPACE / "resources" / "registry" / "test_endpoint_accessibility.yml"
...
def resolve_base(env_cfg: Dict[str, str], path: str) -> str:
    """Return auth_host for /api/auth/* paths, host for everything else."""
    if path.startswith("/api/auth"):
        return env_cfg.get("auth_host", env_cfg["host"])
    return env_cfg["host"]
...
<title>ONDC Registry – Endpoint Accessibility Comparison</title>
<style>
...
/* ── Hero ─────────────────────────────────────────── */
.hero {{ background: linear-gradient(135deg, #134e4a 0%, #0f766e 50%, #0ea5a5 100%); border: 1px solid #0f766e; border-radius: 12px; padding: 32px 36px; margin-bottom: 28px; box-shadow: 0 8px 32px rgba(0,0,0,.45); }}
...
tbody tr:hover {{ background: rgba(14,165,165,.06); }}
...
<div class="page">
    <div class="hero">
        <h1>ONDC Registry – <span>Endpoint Accessibility</span> Comparison</h1>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th colspan="3" class="env-group" style="color:#0ea5a5;">{escape(env_names[0]).upper()}</th>
...
def main() -> None:
    cfg = load_config()
    registry_cfg = cfg.get("ondcRegistry", {})
    environments = registry_cfg.get("environments", {})
    ...
    print(f"Endpoint Accessibility Test — {len(endpoints)} endpoints x {len(env_names)} environments")
    for name in env_names:
    ...
    for ep in endpoints:
        ...
        for env_name in env_names:
            ...
            base = resolve_base(env_cfg, path)
    ...
    report_path = RESULTS_DIR / f"reg_endpoint_accessibility_{ts}.html"
